...
window.sketches['svgUpload'] = function(p) {
    ...
    function renderColorMap(wrap) {
        ...
        var rows = svgColors.map(function(c) {
            ...
            var isSkipped = skip.indexOf(String(resolved).toLowerCase()) !== -1;
            return '<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px;' + (isSkipped ? 'opacity:0.4;' : '') + '">'
                ...
                + '<span style="font-size:11px;color:#666;">' + (isSkipped ? 'skipped \u2014 will not plot' : modeLabel) + '</span>'
                + '</div>';
        }).join('');
        wrap.innerHTML = '<div style="font-size:12px;font-weight:700;color:#555;margin-bottom:4px;">Color Mapping</div>'
            + '<div style="font-size:11px;color:#777;margin-bottom:6px;">Each color found in the uploaded SVG, and the pen it will draw with.</div>'
            + rows;
    }
    ...
    var api = {
        stylePresets: [
            { label: 'Fit, single pen', values: { fitMode: 'fit', recolor: 'single', rotation: 0 } },
            { label: 'Fit, map to pens', values: { fitMode: 'fit', recolor: 'nearest', rotation: 0 } },
            { label: 'Keep original colors', values: { fitMode: 'fit', recolor: 'original', rotation: 0 } }
        ],
        params: paper.buildPaperParams(PARAMS.paperSize, PARAMS.margin).concat([
            ...
            { id: 'recolor', label: 'Recolor', type: 'select', value: 'nearest', group: 'advanced',
              tip: 'Snap to nearest (default) = each SVG color maps to the closest selected pen, so multi-color art splits into separate layers/tool changes. Single pen = every stroke draws with just the first selected pen \u2014 only for intentional monochrome plots. Original = keep the SVG\'s own colors as literal layers.',
        ...
        getPlotColors: function() {
            ...
            var mode = PARAMS.recolor, seen = {}, out = [];
            svgColors.forEach(function(c) {
                var resolved = _pl0tResolvePenFor(c, mode, pens);

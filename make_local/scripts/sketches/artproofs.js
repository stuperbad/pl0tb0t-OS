...
window.sketches['artproofs'] = function(p) {
    ...
    var api = {
        ...
        params: paper.buildPaperParams(PARAMS.paperSize, PARAMS.margin).concat([
            { id: 'layoutMode', label: 'Layout', type: 'select', value: 'scatter',
              tip: 'Scatter places proofs freely. Grid snaps each proof centre to a cell, and the pattern decides which cells stay empty -- that is what makes checkerboards and diagonals.',
              options: [{ value: 'scatter', label: 'Scatter' }, { value: 'grid', label: 'Grid' }] },
            { id: 'gridCols', label: 'Columns', type: 'range', min: 1, max: 12, step: 1, value: 3,
              visibleWhen: { param: 'layoutMode', values: ['grid'] } },
            { id: 'gridRows', label: 'Rows', type: 'range', min: 1, max: 12, step: 1, value: 4,
              visibleWhen: { param: 'layoutMode', values: ['grid'] } },
            { id: 'gridPattern', label: 'Cell pattern', type: 'select', value: 'all',
              visibleWhen: { param: 'layoutMode', values: ['grid'] },
              tip: 'Which cells get a proof. In grid mode the pattern sets the count, so Instances is ignored.',
              options: [
                { value: 'all', label: 'All' },
                { value: 'checker', label: 'Checker' },
                { value: 'altRows', label: 'Alt rows' },
                { value: 'altCols', label: 'Alt cols' },
                { value: 'diagonal', label: 'Diagonal' },
                { value: 'border', label: 'Border' },
                { value: 'custom', label: 'Custom' }
              ] },
            { id: 'gridCells', label: 'Cells', type: 'text', value: '',
              visibleWhen: { param: 'gridPattern', values: ['custom'] },
              tip: 'Cell numbers, row-major and 1-based, so they read the way the grid looks. e.g. 1,3,5,8' },
            { id: 'gridJitter', label: 'Cell jitter', type: 'range', min: 0, max: 100, step: 1, value: 0,
              visibleWhen: { param: 'layoutMode', values: ['grid'] },
              tip: 'How far a proof may wander from its cell centre, as a % of the cell. 0 snaps exactly.' },

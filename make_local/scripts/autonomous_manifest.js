// Auto-generated/updated by the pl0tb0t-friday-art-maker routine.
// Each entry just needs a `file` path under scripts/autonomous_sketches/ —
// name/description/presets are read from the sketch's own
// window.__autonomousSketch export once loaded (see autonomousLoader.js).
window.__autonomousManifest = [
    { file: "scripts/autonomous_sketches/strata.js" },
    { file: "scripts/autonomous_sketches/topo.js" },
    { file: "scripts/autonomous_sketches/truchet.js" },
    { file: "scripts/autonomous_sketches/isocubes.js" },
    { file: "scripts/autonomous_sketches/voronoi.js" },
    { file: "scripts/autonomous_sketches/branches.js" },
    { file: "scripts/autonomous_sketches/spirograph.js" },
    { file: "scripts/autonomous_sketches/moire.js" },
    { file: "scripts/autonomous_sketches/weave.js" },
    { file: "scripts/autonomous_sketches/circuits.js" },
    { file: "scripts/autonomous_sketches/ribbontwist.js" },
    { file: "scripts/autonomous_sketches/cutaway.js" },
    { file: "scripts/autonomous_sketches/turing.js" },
    { file: "scripts/autonomous_sketches/girih.js" },
    { file: "scripts/autonomous_sketches/chladni.js" },
    { file: "scripts/autonomous_sketches/riverdelta.js" },
    { file: "scripts/autonomous_sketches/stitchwork.js" },
    { file: "scripts/autonomous_sketches/ridgeline.js" },
    { file: "scripts/autonomous_sketches/circlepack.js" },
    { file: "scripts/autonomous_sketches/automaton.js" },
    { file: "scripts/autonomous_sketches/dendrite.js" },
    { file: "scripts/autonomous_sketches/phyllotaxis.js" },
    { file: "scripts/autonomous_sketches/penrose.js" },
    { file: "scripts/autonomous_sketches/lsystem.js" },
    { file: "scripts/autonomous_sketches/harmonograph.js" },
    { file: "scripts/autonomous_sketches/globe.js" },
    { file: "scripts/autonomous_sketches/facetgem.js" }
];
